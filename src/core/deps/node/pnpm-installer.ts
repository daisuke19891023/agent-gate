/**
 * pnpm installer implementation.
 *
 * Runs `pnpm install --frozen-lockfile` for CI-mode installation.
 */
 ...
 *
 * Uses `pnpm install --frozen-lockfile` to ensure lockfile integrity.
 */
export async function installWithPnpm(options: InstallOptions): Promise<InstallResult> {
  ...
  // Get pnpm version for reporting
  try {
  ...
  try {
    ...
    const trouble = troubleshootInstall("pnpm", null, message, "", false);
